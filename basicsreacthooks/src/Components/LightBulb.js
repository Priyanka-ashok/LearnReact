...
function LightBulb() {
  const [light, updatelight] = useState(0);
  const turnOnLight = () => updatelight(1);
  const turnOffLight = () => updatelight(0);
  let fillColorToBulb = light === 1 ? "red" : "yellow";
  return (
    <div>
      <div>
        <LightBulbSvg fillColor={fillColorToBulb} />
      </div>
      <button  className="button--click" onClick={turnOffLight}>Off</button>
      <button  className="button--click" onClick={turnOnLight}>On</button>
    </div>
  );
}
